...
#
# These live in `common/tools.py` because they are shared with the plugin harness in `dshpy/`.
# A tool is just a function; which harness calls it, and over which wire protocol, is not its
# concern. See common/tools.py for why calculate() is an AST allowlist and not eval().
# ---------------------------------------------------------------------------
